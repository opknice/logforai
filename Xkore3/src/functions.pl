...
sub initNetworking {
	...
	eval {
		...
		} elsif ($XKore_version eq "hybridge") {   # ใช้ keyword ใหม่เพื่อไม่ชน
			require Network::XKore3;
			$net = Network::XKore3->new;
		}
...
# OpenKore เรียก method นี้ทุก main loop เพื่อดึง packet ที่รอ
# เราคืน $self->{buffer} ที่ _handleServerPacket() สะสมไว้
# แล้วล้างให้ว่างเพื่อรอรับครั้งต่อไป
