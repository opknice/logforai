...
sub _processServerPacket {
    my ($self, $payload) = @_;
    # ส่ง raw packet เข้า OpenKore's packet queue
    # OpenKore จะ dispatch ไปยัง handler ที่ถูกต้องเอง
    # เช่น ถ้าเป็น 0x0AC4 (CHAR_INFO) จะเรียก handler_charInfo()
    # ถ้าเป็น 0x0087 (MOVE_ACK) จะ update position ของ character ใน memory
    $messageSender->encryptMessageID(\$payload);  # ถ้า server encrypt
    Globals::processServerPacket($payload);
}
